            ...
            <div className='flex w-[80%] justify-between'>
                <div className='flex gap-2'>
                    {socialMedia.map((each,index)=>
                        <a key={index} href={each === 'linkedin' ? 'https://www.linkedin.com/in/ruchi-sharma-91a40428a' : `https://www.${each}.com`} target="_blank" className='text-[16px] text-silver bg-gray-200 w-[25px] rounded-md hover:bg-salmon hover:text-white'><i className={`fa-brands fa-${each}`}></i></a>
                    )}
        ...
        <div className='w-[100%] h-auto flex flex-col justify-between items-center border-b-[1px]'>
            <div className='flex justify-evenly items-center w-[100%] flex-col sm:flex-row gap-2 sm:gap-0'>
                <div className='w-[80%] flex justify-between items-center flex-col sm:flex-row sm:gap-0'>
                    <div>
                        <Link className='mr-2.5 text-[26px] font-bold' href={"/"}>Auralyze</Link>
                    </div>
