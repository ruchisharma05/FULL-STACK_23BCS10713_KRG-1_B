      ...
      {(dataChecked.current && found.current && loggedIn.current) && <div className="2xl:w-4/6 mx-auto p-4 bg-white rounded-lg mt-10 mb-10">
        ...
        <div className="flex flex-col border-[1px] rounded-xl px-6 py-6 gap-5">
          <div className='flex justify-between'>
            ...
            <p className='text-xl font-semibold'>₹{parseFloat(formattedSubTotal) * 60}</p>
          </div>
          <div className='flex justify-between'>
            <p className='text-lg text-silver'>Shipping Charge</p>
            <p className='text-xl font-semibold'>₹{parseFloat(formattedShipping) * 60}</p>
          </div>
          {dataVar.current.paymentmethod==='Payment on Delivery' &&
          <div className='flex justify-between'>
            <p className='text-lg text-silver'>Payment Processing Charge</p>
            <p className='text-xl font-semibold'>₹{paymentCharge.current * 60}</p>
          </div>
          }
          <div className='flex justify-between'>
            <p className='text-lg text-silver'>Taxes</p>
            <p className='text-xl font-semibold'>₹{parseFloat(formattedTaxes) * 60}</p>
          </div>
          <div className='flex justify-between'>
            <p className='text-lg text-silver'>Discount</p>
            <p className='text-xl font-semibold'>₹{parseFloat(formattedDiscount) * 60}</p>
          </div>
          <div className='w-full h-[1px] bg-gray-100'></div>
          <div className='flex justify-between'>
            <p className='text-2xl font-medium'>Total</p>
            <p className='font-semibold text-2xl'>₹{parseFloat(formattedTotalAmount) * 60}</p>
          </div>
        </div>
      </div>}
      </div>
