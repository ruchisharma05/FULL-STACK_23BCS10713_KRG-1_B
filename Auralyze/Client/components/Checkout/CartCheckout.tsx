        ...
        <form action="/" id='informational-form' onSubmit={createOrder} className="min-w-0 flex-1 space-y-8">
            ...
            {dataVar.current.map((each,index)=><div key={index} className="p-4 mb-8 flex flex-col gap-4">
                <div className="flex border-[1px] min-h-[150px] rounded-xl px-6 py-6 items-center mb-4">
                    ...
                    <div>
                        <p className='font-medium text-3xl'>₹{parseFloat(each.discount) * 60}</p>
                        <p className='font-medium text-xl line-through'>₹{parseFloat(each.price) * 60}</p>
                    </div>
                    </div>
                </div>
            </div>)}
        ...
        <div className="mt-6 w-full space-y-6 sm:mt-8 lg:mt-0 lg:max-w-xs xl:max-w-md">
            <div className="flow-root">
                <div className="-my-3 divide-y divide-gray-200 dark:divide-gray-800">
                    ...
                    <dd className="text-base font-medium text-gray-900 dark:text-white">₹{parseFloat(formattedSubTotal) * 60}</dd>
                    </dl>

                    <dl className="flex items-center justify-between gap-4 py-3">
                    <dt className="text-base font-normal text-gray-500 dark:text-gray-400">Shipping Charge</dt>
                    <dd className="text-base font-medium text-gray-900">₹{parseFloat(formattedShipping) * 60}</dd>
                    </dl>
                    ...
                    <dd className="text-base font-medium text-gray-900 dark:text-white">₹{paymentCharge * 60 * dataVar.current.length}</dd>
                    </dl>}

                    <dl className="flex items-center justify-between gap-4 py-3">
                    <dt className="text-base font-normal text-gray-500 dark:text-gray-400">Taxes</dt>
                    <dd className="text-base font-medium text-gray-900 dark:text-white">₹{parseFloat(formattedTaxes) * 60}</dd>
                    </dl>

                    <dl className="flex items-center justify-between gap-4 py-3">
                    <dt className="text-base font-normal text-gray-500 dark:text-gray-400">Discount</dt>
                    <dd className="text-base font-medium text-green-500 dark:text-white">₹{parseFloat(formattedDiscount) * 60}</dd>
                    </dl>

                    <dl className="flex items-center justify-between gap-4 py-3">
                    <dt className="text-base font-bold text-gray-900 dark:text-white">Total</dt>
                    <dd className="text-base font-bold text-gray-900 dark:text-white">₹{parseFloat(formattedTotalAmount) * 60}</dd>
                    </dl>
                </div>
            </div>
